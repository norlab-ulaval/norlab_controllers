...
class StochasticLinearMPC(Controller):
    def __init__(self, parameter_map):
        ...
        self.gain_distance_to_goal_linear = parameter_map[
            "gain_distance_to_goal_linear"
        ]
        ...
        self.ancillary_gain_array = np.array(
            [self.ancillary_gain_linear, self.ancillary_gain_angular]
        ).reshape(2, 1)
        ...
        self.orthogonal_projection_ids_horizon = np.zeros(self.horizon_length).astype(
            "int32"
        )
        ...
        self.ideal_diff_drive = Ideal_diff_drive(
            self.wheel_radius, self.baseline, 1 / self.rate
        )
        ...
        self.max_wheel_vel = self.ideal_diff_drive.compute_wheel_vels(
            np.array([self.maximum_linear_velocity, 0])
        )[0]
        ...
        self.straight_line_input[self.horizon_length :] = np.full(
            self.horizon_length, 2.0
        )
        ...
        self.x_k = (
            self.casadi_x
            + cas.mtimes(self.R, cas.mtimes(self.J, self.casadi_u)) / self.rate
        )
        self.single_step_pred = cas.Function(
            "single_step_pred", [self.casadi_x, self.casadi_u], [self.x_k]
        )
        ...
        self.x_ref[1, :] = self.x_ref_flat[
            self.horizon_length : 2 * self.horizon_length
        ]
        self.x_ref[2, :] = self.x_ref_flat[
            2 * self.horizon_length : 3 * self.horizon_length
        ]
        ...
        for i in range(1, self.horizon_length):
            self.x_horizon_list.append(
                self.single_step_pred(
                    self.x_horizon_list[i - 1], self.u_horizon[i - 1, :]
                )
            )
            x_error = self.x_ref[:, i] - self.x_horizon_list[i]
            state_cost = cas.mtimes(
                cas.mtimes(x_error.T, self.cas_state_cost_matrix), x_error
            )
            u_error = self.u_ref[i - 1, :] - self.u_horizon[i - 1, :]
            input_cost = cas.mtimes(
                cas.mtimes(u_error, self.cas_input_cost_matrix), u_error.T
            )
            self.prediction_cost = self.prediction_cost + state_cost + input_cost

        self.x_horizon = cas.hcat(self.x_horizon_list)
        self.horizon_pred = cas.Function(
            "horizon_pred", [self.x_0, self.u_horizon_flat], [self.x_horizon]
        )
        self.pred_cost = cas.Function(
            "pred_cost", [self.u_horizon_flat], [self.prediction_cost]
        )
        ...
        self.optim_problem = {
            "f": self.prediction_cost,
            "x": self.u_horizon_flat,
            "p": self.nlp_params,
        }
        self.nlpsol_opts = {
            "verbose_init": False,
        ...
        self.optim_problem_solver = cas.nlpsol(
            "optim_problem_solver", "ipopt", self.optim_problem, self.nlpsol_opts
        )
    ...
    def predict_horizon(self, init_state, input_array):
        ...
        ) = self.full_body_blr_model.predict_horizon_from_body_idd_vels(
            body_vels_horizon, init_state, self.initial_state_covariance
        )
    ...
    def compute_orthogonal_projection(self, state):
        (
            self.orthogonal_projection_dists,
            self.orthogonal_projection_ids,
        ) = self.path.compute_orthogonal_projection(
            state[:2], self.last_path_pose_id, self.query_knn, self.query_radius
        )
        for i in range(0, self.orthogonal_projection_ids.shape[0]):
            if (
                np.abs(self.orthogonal_projection_ids[i] - self.last_path_pose_id)
                <= self.id_window_size
            ):
        ...
        for i in range(0, self.horizon_length):
            if target_trajectory_path_id + 1 == self.path.poses.shape[0]:
                for j in range(i, self.horizon_length):
                    self.target_trajectory[:2, j] = self.path.poses[
                        target_trajectory_path_id
                    ][:2]
                    self.target_trajectory[2, j] = self.path.angles[
                        target_trajectory_path_id
                    ]
                break
            target_displacement += target_displacement_rate
            path_displacement = (
                self.path.distances_to_goal[target_trajectory_path_id]
                - self.path.distances_to_goal[target_trajectory_path_id + 1]
            )
    ...
    def compute_orthogonal_projections(self, prediction_means):
        (
            orthogonal_projection_dists,
            orthogonal_projection_ids,
        ) = self.path.pose_kdtree.query(
            prediction_means[:2, :].T,
            k=self.query_knn,
            distance_upper_bound=self.query_radius,
        )
        for i in range(0, self.horizon_length):
            for j in range(0, orthogonal_projection_ids.shape[1]):
                if (
                    np.abs(orthogonal_projection_ids[i, j] - self.last_path_pose_id)
                    <= self.id_window_size
                ):
                    self.orthogonal_projection_ids_horizon[i] = int(
                        orthogonal_projection_ids[i, j]
                    )
                    self.orthogonal_projection_dists_horizon[
                        i
                    ] = orthogonal_projection_dists[i, j]
                    break
    ...
    def compute_distance_to_goal(self, state, orthogonal_projection_id):
        self.euclidean_distance_to_goal = np.linalg.norm(
            self.path.poses[-1, :2] - state[:2]
        )
        self.distance_to_goal = self.path.distances_to_goal[orthogonal_projection_id]
        return None

    def compute_horizon_cost(
        self, prediction_means, prediction_covariances, input_array
    ):
        prediction_cost = 0
        for i in range(0, self.horizon_length):
            state_error = (
                prediction_means[:, i]
                - self.path.planar_poses[self.orthogonal_projection_ids_horizon[i]]
            )
            state_cost_timestep = (
                state_error @ self.state_cost_matrix @ state_error.T
                + np.trace(self.state_cost_matrix @ prediction_covariances[:, :, i])
            )
    ...
    def predict_then_compute_cost(self, input):
        ...
        self.prediction_means, self.prediction_covariances = self.predict_horizon(
            self.init_state, input
        )
        self.compute_orthogonal_projections(self.prediction_means)
        return self.compute_horizon_cost(
            self.prediction_means, self.prediction_covariances, input
        )
    ...
    def compute_objective(self, input):
        ...
        prediction_means, prediction_covariances = self.predict_horizon(
            self.init_state, self.nd_input_array
        )
        self.compute_orthogonal_projections(prediction_means)
        cost = self.compute_horizon_cost(
            prediction_means, prediction_covariances, self.nd_input_array
        )
        return cost
    ...
    def compute_command_vector(self, state):
        ...
        nlp_params = np.concatenate(
            (self.planar_state, self.target_trajectory.flatten("C"))
        )
        ...
        self.previous_input_array[0, :] = np.array(
            self.optim_control_solution[: self.horizon_length]
        ).reshape(self.horizon_length)
        self.previous_input_array[1, :] = np.array(
            self.optim_control_solution[self.horizon_length :]
        ).reshape(self.horizon_length)
        self.optimal_left = self.optim_control_solution[0]
        self.optimal_right = self.optim_control_solution[self.horizon_length]
        wheel_input_array = np.array(
            [
                self.optim_control_solution[0],
                self.optim_control_solution[self.horizon_length],
            ]
        ).reshape(2, 1)
        body_input_array = self.ideal_diff_drive.compute_body_vel(
            wheel_input_array
        ).astype("float64")

        optim_trajectory = self.horizon_pred(
            self.planar_state, self.optim_control_solution
        )
        ...
        self.previous_input_array[0, :] = self.optim_solution_array[
            : self.horizon_length
        ].reshape(self.horizon_length)
        self.previous_input_array[1, :] = self.optim_solution_array[
            self.horizon_length :
        ].reshape(self.horizon_length)
